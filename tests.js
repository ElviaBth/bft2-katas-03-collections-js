...
describe("Operaciones iterables", () => {
    ...
    it("map nos permite realizar operaciones elemento a elemento (2)", () => {
        ...
        function aplicar(name){
            return name + ", te elijo a ti!"
        }

        let result = list.map(aplicar);
    ...
    it("Busca la palabra oculta",()=>{
        let secretWord=["España", "La Teixonera", "Vizkaya", "Icaria", "Almeria"];

        //Utiliza map para descubrir la palabra oculta creando un array con la primera letra de cada palabra.
        //Utiliza join('') para eliminar los espacios entre el texto oculto.
        let result= secretWord.map(firstLetter=> firstLetter[0]);
        

        expect(result).to.have.same.members(['E','L','V','I','A'])
    })
})